...
class Path:
    def __init__(self):
        self.path = list(range(0,n))
        self.path.remove(start)         #[1,2,3,4,5]
        random.shuffle(self.path) # [2,4,3,5,1]
        self.fitness = self.getFitness()
        self.score = 1/self.fitness
        # print(self.path)
    ...
    def getFitness(self):
        ...
        for i in range(len(self.path)):
            pcost += graph[pre][self.path[i]]
            pre = self.path[i]
        pcost += graph[self.path[-1]][start]
        self.fitness = pcost
        self.score = 1 / self.fitness
    ...
    def mutate(self):
        if random.random() < 0.10:
            ...
            self.path[x],self.path[y] = self.path[y],self.path[x]
            self.getFitness()
